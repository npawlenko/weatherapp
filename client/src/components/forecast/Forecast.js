...
import {findCityByName, currentForecast} from "../../services/weatherAppService";
...
function Forecast(props) {
    ...
    useEffect( () => {
        ...
        if(!getForecast && props.city) {
            async function fetchCityAndForecast() {
                try {
                    const foundCities = await findCityByName(props.city);
    ...
    function toggleFavouriteCity(e) {
        ...
        e.target.classList.toggle("forecast-star-favourite");localStorage.setItem(Constants.LOCALSTORAGE_FAVOURITE_CITIES, JSON.stringify(favouriteCities));
    }
